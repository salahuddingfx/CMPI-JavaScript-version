...
import {
  getSubjectName,
  detectDepartmentFromSubjects,
} from "@/utils/btebSubjectCodes";
...
function GradePill({ grade }) {
  return (
    <span
      className={`inline-block rounded-full px-2.5 py-0.5 text-xs font-black ${gradeColor[grade] ?? "bg-muted text-muted-foreground"}`}
    >
      {grade}
    </span>
  );
...
function parseSubjectSuffix(subjectCode) {
...
function semIndex(s) {
  const idx = SEM_ORDER.findIndex((x) =>
    (s ?? "").toLowerCase().startsWith(x.charAt(0)),
  );
  return idx === -1 ? 99 : idx;
}

// Infer most accurate department from all records for this student
function computeDepartment(records) {
  ...
  const stored = records.find(
    (r) =>
      r.department &&
      r.department !== "General Technology" &&
      r.department !== "Auto Detect",
  );
...
const mockResults = {
  "CMPI-2023-0456": {
    ...
    roll: "232345",
    reg: "2023456789",
    institute: "Cox's Bazar Model Polytechnic Institute",
...
export function Results() {
  const [searchMode, setSearchMode] = useState("transcript");
  const [query, setQuery] = useState("");
  const [result, setResult] = useState(null);
  const [btebResults, setBtebResults] = useState([]);
      ...
      try {
        ...
        setError(
          err.response?.data?.message ||
            "Failed to retrieve results from BTEB board database.",
        );
      } finally {
        setLoading(false);
      }
  ...
  return (
    <PageTransition>
      <SEO
        ...
        description="Check your semester examination BTEB results at CMPI."
      />
      ...
      <section className="container section-pad">
        ...
        <div className="mx-auto max-w-xl flex gap-2 mb-8 border p-1.5 rounded-xl bg-muted/30">
          ...
          >
            <span className="hidden sm:inline">Student Transcript (Student ID)</span>
        ...
        <div className="mx-auto max-w-xl">
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            {searchMode === "transcript" ? (
              <>
                Try: <span className="font-mono">CMPI-2023-0456</span> or{" "}
                <span className="font-mono">CMPI-2023-0312</span>
              </>
            ) : (
          ...
          <div className="mx-auto mt-10 max-w-3xl space-y-6">
            {btebResults.length > 0 ? (
              (() => {
                ...
                const sorted = [...btebResults].sort(
                  (a, b) => semIndex(a.semester) - semIndex(b.semester),
                );
              ...
              <div className="rounded-sm border bg-card p-10 text-center shadow-sm">
                ...
                <p className="font-bold text-foreground">
                  No board results found
                </p>
                <p className="mt-1 text-sm text-muted-foreground">
                  No BTEB board records match roll number "{query}". Please
                  check the roll and try again.
                </p>
              </div>
            )}
          </div>
            ...
            {result ? (
              <div className="space-y-6">
                ...
                {result.semesters.map((sem) => (
                  <div
                    key={sem.semester}
                    className="rounded-sm border bg-card shadow-sm overflow-hidden mb-4"
                  >
                    <button
                      type="button"
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div className="text-left">
                          ...
                          <p className="text-xs text-muted-foreground">
                            Session: {sem.session} · Holding Year:{" "}
                            {sem.holdingYear}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        ...
                        <span
                          className={`rounded-full px-3 py-1 text-xs font-black ${sem.status === "Passed" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-red-100 text-red-700"}`}
                        >
                          {sem.status}
                        </span>
                      </div>
                          ...
                          <tbody>
                            {sem.courses.map((c, i) => (
                              ...
                              >
                                ...
                                <td className="px-4 py-2.5 text-xs text-green-600 font-semibold">
                                  {c.remarks}
                                </td>
                              </tr>
                            ))}
                          ...
                          <tfoot>
                            <tr className="bg-primary/5 border-t font-black">
                              ...
                              <td className="px-4 py-2 text-xs text-green-600">
                                {sem.earnedCredit}/{sem.totalCredit} credits
                                earned
                              </td>
                            </tr>
                          </tfoot>
              ...
              <div className="rounded-sm border bg-card p-10 text-center shadow-sm">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  No records match Student ID "{query}". Please verify and try
                  again.
                </p>
              </div>
            )}
