...
export function Syllabus() {
  ...
  const [data, setData] = useState(syllabuses);
  const [preview, setPreview] = useState(null);
  const fileInputRefs = useRef({});

  const handleFileUpload = (dept, semIndex, file) => {
    ...
    setData((prev) => ({
      ...prev,
      [dept]: prev[dept].map((s, i) =>
        i === semIndex ? { ...s, pdfUrl: url } : s,
      ),
    }));
  };

  const handleRemovePdf = (dept, semIndex) => {
    const currentUrl = data[dept][semIndex]?.pdfUrl;
    if (currentUrl) URL.revokeObjectURL(currentUrl);
    setData((prev) => ({
      ...prev,
      [dept]: prev[dept].map((s, i) =>
        i === semIndex ? { ...s, pdfUrl: null } : s,
      ),
    }));
  };

  const depts = Object.keys(syllabuses);

  return (
    <PageTransition>
      <SEO
        title="Syllabus & Curriculum"
        description="Curriculum and syllabus for all diploma programs at CMPI."
      />
        ...
        <div className="mx-auto max-w-3xl space-y-6">
          {data[activeDept].map((sem, semIndex) => {
            ...
            return (
              <div
                key={sem.sem}
                className="rounded-sm border bg-card shadow-sm overflow-hidden"
              >
                {/* Semester Header */}
                    ...
                    {sem.pdfUrl ? (
                      <>
                        ...
                        <div className="flex flex-col gap-2 w-full">
                          ...
                          <a
                            href={sem.pdfUrl}
                            download={`syllabus-${activeDept}-${sem.sem}.pdf`}
                          >
                            <Button
                              size="sm"
                              variant="outline"
                          ...
                          <Button
                            ...
                            onClick={() =>
                              handleRemovePdf(activeDept, semIndex)
                            }
                          >
                      ...
                      <>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (file)
                              handleFileUpload(activeDept, semIndex, file);
                            e.target.value = "";
                          }}
                        />
