...
const links = [
  ...
  { label: "Civil Technology", href: "/academics/civil-technology" },
  {
    ...
    href: "/academics/computer-science-technology",
  },
...
export function Sitemap() {
  return (
    <PageTransition className="container section-pad">
      <SEO
        title="Sitemap"
        description="Full page list for Cox's Bazar Model Polytechnic Institute."
      />
      <article className="mx-auto max-w-5xl rounded-sm border bg-card p-6 shadow-sm sm:p-10">
        <SectionHeader
          title="Sitemap"
          description="All accessible pages on the CMPI website."
          align="left"
          className="mb-8"
        />
        <div className="grid gap-3 md:grid-cols-2">
          {links.map((link) => (
            <Link
              key={link.href}
              to={link.href}
              className="flex items-center justify-between rounded-sm border border-border/60 bg-muted/60 px-4 py-3 font-semibold transition hover:border-primary hover:text-primary"
            >
              {link.label}
