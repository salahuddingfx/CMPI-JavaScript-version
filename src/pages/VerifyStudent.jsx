    ...
    verifyStudent(studentId)
      ...
      .catch((err) => {
        if (err.response?.status === 404) {
          setError(
            "This Student ID is not registered in the CMPI institutional database. The credential could be invalid or counterfeit.",
          );
        } else {
          setError(
            "Could not complete verification process due to a server error.",
          );
        }
      })
  ...
  return (
    <>
      <SEO
        ...
        description="Scan and verify student credentials at Cox's Bazar Model Polytechnic Institute."
      />
      ...
      <div className="container flex min-h-[70vh] flex-col items-center justify-center py-10 sm:py-16">
        <div className="w-full max-w-md">
          ...
          {loading ? (
            ...
            <Card className="border-destructive/30 bg-destructive/5 text-destructive shadow-md">
              ...
              <CardContent className="text-center text-sm leading-relaxed text-destructive/90 px-6 pb-6">
                ...
                <div className="mt-6 rounded-lg bg-destructive/10 p-4 text-xs font-mono text-left border border-destructive/20 space-y-1">
                  <p className="font-bold uppercase tracking-wider text-[9px] text-destructive/70">
                    Scanned Parameter
                  </p>
                  <p className="truncate text-destructive font-semibold">
                    {studentId}
                  </p>
                </div>
              </CardContent>
              <div className="bg-destructive/10 flex justify-center py-4 rounded-b-xl border-t border-destructive/20">
                <p className="text-[10px] text-destructive/70 font-semibold uppercase tracking-widest flex items-center gap-1.5">
                  <AlertTriangle className="h-3.5 w-3.5" /> Unauthorized ID Card
                  Warning
                </p>
              </div>
            </Card>
            ...
            <Card
              className={`border-emerald-500/30 bg-emerald-500/5 shadow-lg border-2 ${
            ...
            >
              <CardHeader className="text-center relative">
                ...
                <div className="mx-auto relative">
                  <div
                    className={`mx-auto h-20 w-20 overflow-hidden rounded-full border-4 shadow-md ${
                  ...
                  {/* Verified Icon Badge */}
                  <div
                    className={`absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/3 flex items-center gap-1 rounded-full px-3 py-0.5 text-[10px] font-black uppercase tracking-wider shadow-sm text-white ${
                      student.status === "active" ||
                      student.status === "approved"
                        ? "bg-emerald-600"
                        : "bg-amber-600"
              ...
              <CardContent className="space-y-4 px-6 pb-6">
                ...
                {!(
                  ...
                  <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-700 dark:text-amber-400 font-medium flex items-start gap-2 leading-relaxed">
                    ...
                    <p>
                      <strong>Status Alert:</strong> This student status is
                      marked as <strong>{student.status}</strong>. Please
                      contact the CMPI Registrar Office for verification of
                      enrollment status.
                    </p>
                  </div>
                )}
              ...
              <div
                className={`flex flex-col items-center gap-2 py-4 rounded-b-xl border-t text-xs font-medium text-slate-500 ${
                  student.status === "active"
