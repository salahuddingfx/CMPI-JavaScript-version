...
import { getEmailsForUser } from "@/data/mockStudentData";
...
export function StudentWebmail() {
  ...
  const contacts = useMemo(
    () => allContacts.filter((c) => c.email !== userEmail),
    [userEmail],
  );
  ...
  const currentLabel = selected
    ? (emailLabels[selected.id] ?? selected.label)
    : null;
  ...
  const spamEmail = useCallback(
    (id) => {
      setEmails((prev) =>
        prev.map((e) => (e.id === id ? { ...e, folder: "spam" } : e)),
      );
      setSelectedId(null);
  ...
  const markAllAsRead = useCallback(() => {
    setEmails((prev) =>
      prev.map((e) => (e.folder === activeFolder ? { ...e, read: true } : e)),
    );
    addToast("All emails marked as read");
  }, [activeFolder, addToast]);

  const openCompose = useCallback((mode, replyTo) => {
    setComposeMode(mode);
  ...
  const handleComposeBodyChange = useCallback(
    (value) => {
    ...
    const handler = (e) => {
      ...
      if (e.key === "j" || e.key === "J") {
        ...
        if (currentIndex < list.length - 1)
          openEmail(list[currentIndex + 1].id);
  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        eyebrow="Webmail"
        title={`${userName}'s mailbox`}
        description={`${userEmail} — Press J/K navigate, R reply, D delete, S star`}
        align="left"
      />
      ...
      {toasts.length > 0 && (
        <div className="fixed right-4 top-4 z-[70] space-y-2">
          {toasts.map((t) => (
            <div
              key={t.id}
              className={`rounded-sm px-4 py-3 text-sm font-semibold text-white shadow-lg ${t.type === "success" ? "bg-green-600" : "bg-blue-600"}`}
            >
              {t.message}
            </div>
          ))}
            ...
            {folders.map((folder) => {
              ...
              return (
                <button
                  key={folder.key}
                  type="button"
                  className={`flex w-full items-center gap-2 rounded-sm px-3 py-2 text-left text-sm font-semibold transition ${activeFolder === folder.key ? "bg-primary text-white" : "hover:bg-background"}`}
                  onClick={() => {
                ...
                >
                  ...
                  {count > 0 && (
                    <span
                      className={`ml-auto rounded-full px-2 py-0.5 text-xs font-bold ${activeFolder === folder.key ? "bg-white/20 text-white" : "bg-primary/10 text-primary"}`}
                    >
                      {count}
                    </span>
                  )}
                ...
                filtered.map((email) => {
                  ...
                  return (
                    <button
                      key={email.id}
                      type="button"
                      className={`flex w-full flex-col gap-1 px-4 py-3 text-left transition hover:bg-background ${selectedId === email.id ? "bg-background" : ""} ${!email.read ? "border-l-2 border-primary" : ""}`}
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          >
                            <Star
                              className={`h-3.5 w-3.5 ${email.starred ? "fill-yellow-500 text-yellow-500" : "text-muted-foreground"}`}
                            />
                          </button>
                          <span
                            className={`text-sm ${!email.read ? "font-bold" : "font-semibold"}`}
                          >
                            {email.from.replace(/@cmpi\.edu\.bd$/, "")}
                          </span>
                        </div>
                      ...
                      <p
                        className={`text-sm ${!email.read ? "font-bold" : "font-medium"}`}
                      >
                        {email.subject}
                      </p>
                      <p className="text-xs text-muted-foreground">
                      ...
                      <div className="flex items-center gap-2">
                        {lbl && (
                          <span
                            className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${labelColors[lbl] ?? ""}`}
                          >
                            {lbl}
                          </span>
                        )}
              ...
              <div className="space-y-4">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    >
                      <Star
                        className={`h-4 w-4 ${selected.starred ? "fill-yellow-500 text-yellow-500" : ""}`}
                      />
                    </Button>
                    <div className="relative group">
                      ...
                      <div className="absolute right-0 top-full z-10 mt-1 hidden w-36 rounded-sm border bg-card p-1 shadow-lg group-hover:block">
                        {["work", "personal", "urgent"].map((l) => (
                          ...
                          >
                            <span
                              className={`h-2 w-2 rounded-full ${labelColors[l]}`}
                            />
                        ...
                        <button
                          type="button"
                          className="w-full rounded-sm px-3 py-1.5 text-left text-xs text-muted-foreground hover:bg-muted"
                          onClick={() => setLabel(selected.id, null)}
                        >
                          Remove label
                        </button>
                      </div>
                ...
                <div className="max-h-[360px] overflow-auto rounded-sm border bg-card p-4">
                  {selected.body === null ? (
                    <div className="flex flex-col items-center justify-center py-10 gap-2">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Fetching email content from mail server...
                      </p>
                    </div>
                  ) : (
      ...
      {composeOpen && (
        <div
          className={`fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4 ${fullscreenCompose ? "items-stretch" : ""}`}
        >
          <div
            className={`flex flex-col rounded-sm border bg-card shadow-2xl ${fullscreenCompose ? "h-full w-full" : "w-full max-w-2xl"}`}
          >
            <div className="mb-4 flex items-center justify-between border-b px-6 py-4">
              ...
              <div className="flex items-center gap-2">
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => setFullscreenCompose(!fullscreenCompose)}
                >
                  {fullscreenCompose ? (
