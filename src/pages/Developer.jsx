...
const timeline = [
  ...
  },
  {
    ...
    description:
      "Adding charts, dark mode, search, SEO optimization, and performance improvements.",
  },
];
...
const testimonials = [
  {
    quote:
      "Salah delivered an exceptional website that exceeded our expectations. The admin panel is intuitive and the student portal is a game-changer.",
    name: "Principal, CMPI",
    role: "Institute Administration",
  },
  {
    quote:
      "Professional, creative, and technically brilliant. The admission system alone saved us hundreds of hours of manual work.",
    name: "Faculty Member, CMPI",
    role: "Academic Department",
  },
  {
    quote:
      "Working with Salah was a pleasure. He understood our vision and translated it into a beautiful, functional website.",
    ...
    <PageTransition className="container section-pad">
      ...
      <article className="mx-auto max-w-4xl space-y-12">
        ...
        <div className="rounded-sm border bg-card p-6 shadow-sm sm:p-10">
          <SectionHeader
            eyebrow="Developer"
            title="Salah Uddin Kader"
            description="The mind behind this website."
            align="center"
            className="mb-10"
          />
          ...
          <div className="flex flex-col items-center gap-8 md:flex-row md:items-start md:gap-10">
            <div className="shrink-0">
              <img
                ...
                className="h-36 w-36 rounded-full object-cover ring-4 ring-primary/20 shadow-lg"
              />
            </div>
            <div className="flex-1 text-center md:text-left">
              ...
              <p className="mt-4 leading-relaxed text-muted-foreground">
                A passionate full-stack developer with a love for clean code and
                modern web experiences. From concept to deployment, I craft
              ...
              <p className="mt-3 leading-relaxed text-muted-foreground">
                ...
                , a creative digital agency focused on building innovative
                products and empowering businesses through technology.
              </p>
              <div className="mt-6 flex flex-wrap justify-center gap-3 md:justify-start">
                {socialLinks.map((link) => (
                  <a
                    ...
                    className="flex items-center gap-2 rounded-full border border-border/60 bg-muted/60 px-4 py-2 text-sm font-medium text-muted-foreground transition hover:border-primary hover:text-primary"
                  >
                    {link.icon}
                    {link.label}
                  </a>
        ...
        <div className="rounded-sm border bg-card p-6 shadow-sm sm:p-10">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {techStack.map((tech) => (
              <span
                key={tech}
                className="rounded-full border border-border/60 bg-muted/60 px-3 py-1 text-xs font-medium text-muted-foreground transition hover:border-primary hover:text-primary"
              >
                {tech}
              </span>
            ))}
        ...
        <div className="rounded-sm border bg-card p-6 shadow-sm sm:p-10">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {projects.map((project) => (
              <div
                key={project.title}
                className="rounded-sm border border-border/60 bg-muted/30 p-5 transition hover:shadow-md"
              >
                <div className="flex items-center justify-between">
                  <h4 className="font-bold text-foreground">{project.title}</h4>
                  <span
                    className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full ${project.status === "Live" ? "bg-green-500/10 text-green-600" : "bg-muted text-muted-foreground"}`}
                  >
                    {project.status}
                  </span>
                </div>
        ...
        <div className="rounded-sm border bg-card p-6 shadow-sm sm:p-10">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {testimonials.map((t, i) => (
              <div
                key={i}
                className="rounded-sm border border-border/60 bg-muted/30 p-5"
              >
                <svg
                  viewBox="0 0 24 24"
                  fill="currentColor"
        ...
        <div className="rounded-sm border bg-card p-6 shadow-sm sm:p-10 text-center">
          ...
          <p className="mt-3 text-muted-foreground">
            A digital agency building modern web experiences, mobile apps, and
            creative solutions.
          </p>
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="mt-4 inline-flex items-center gap-2 rounded-sm bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground transition hover:bg-primary/90"
          >
            Visit Website
            <svg
              viewBox="0 0 24 24"
