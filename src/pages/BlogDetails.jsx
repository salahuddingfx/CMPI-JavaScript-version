...
export function BlogDetails() {
  ...
  const relatedPosts = post.relatedIds
    ? data.blogs.filter((item) => post.relatedIds?.includes(item.id))
    : [];
      ...
      <section className="container section-pad">
        <article className="mx-auto max-w-4xl rounded-sm border bg-card p-6 shadow-sm sm:p-10">
          <div className="mb-5 flex flex-wrap gap-2">
            <span className="rounded-sm bg-primary/10 px-3 py-1 text-sm font-bold text-primary">
              {post.category}
            </span>
            <span className="rounded-sm bg-muted px-3 py-1 text-sm text-muted-foreground">
              {post.readTime}
            </span>
          </div>
          <SectionHeader
