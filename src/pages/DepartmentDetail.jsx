    ...
    <PageTransition>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            <div className="lg:col-span-2">
              <h2 className="text-3xl font-black mb-6 text-slate-900 dark:text-white">Department Overview</h2>
                ...
                {labs.length > 0 && (
                  <div className="bg-slate-50 dark:bg-slate-900 p-8 rounded-2xl border border-slate-100 dark:border-slate-800">
                    ...
                    <ul className="space-y-3 font-semibold text-sm">
                      {labs.map((lab, i) => (
                        <li key={i} className="text-gray-600 dark:text-slate-400 flex items-center gap-2">
            ...
            <div className="space-y-8">
              ...
              <div className="bg-white dark:bg-slate-950 border border-slate-100 dark:border-slate-800 p-8 rounded-2xl shadow-sm">
                <h3 className="text-xl font-bold mb-6 text-slate-900 dark:text-white border-b dark:border-slate-800 pb-4">Downloads</h3>
      ...
      {subjectsLoading ? (
        <section className="container mx-auto px-4 py-12">
          <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-card p-8 text-center text-muted-foreground">Loading curriculum...</div>
        </section>
      ) : availableSemesters.length > 0 ? (
        <section className="container mx-auto px-4 py-12 border-t dark:border-slate-800">
          <h2 className="text-3xl font-black text-slate-900 dark:text-white mb-2">Subjects by Semester</h2>
          <p className="text-slate-500 dark:text-slate-400 mb-6">Select a semester tab below to view course codes and credit metrics.</p>
          ...
          {groupedSubjects[openSem] && (
            <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 shadow-sm overflow-hidden">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-800 border-b dark:border-slate-700 text-slate-500 dark:text-slate-400">
                ...
                <tbody className="divide-y dark:divide-slate-800 font-semibold">
                  {groupedSubjects[openSem].map((sub, i) => (
                    <tr key={sub.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50">
                      ...
                      <td className="px-5 py-3.5 text-slate-800 dark:text-slate-200">{sub.subject_name}</td>
                      <td className="px-5 py-3.5 text-center">{sub.credit ?? "—"}</td>
                      <td className="px-5 py-3.5 text-center hidden sm:table-cell text-slate-500 dark:text-slate-400">{sub.theory_marks || 0}</td>
                      <td className="px-5 py-3.5 text-center hidden sm:table-cell text-slate-500 dark:text-slate-400">{sub.practical_marks || 0}</td>
      ...
      {deptFaculty.length > 0 && (
        <section className="py-20 bg-slate-50 dark:bg-slate-900 border-t dark:border-slate-800">
          <div className="container mx-auto px-4">
            <div className="text-center mb-16">
              <h2 className="text-3xl font-black text-slate-900 dark:text-white">Department Faculty</h2>
              <p className="text-gray-500 dark:text-slate-400 mt-2">Meet the experts leading the {department.name || department.title} department.</p>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              {deptFaculty.map((member) => (
                <div key={member.id} className="bg-white dark:bg-slate-950 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all group border border-slate-100 dark:border-slate-800 flex flex-col h-full">
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    ...
                    <p className="text-xs text-gray-500 dark:text-slate-400 mt-auto pt-2 border-t dark:border-slate-800 font-semibold">{member.qualification}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {achievements.length > 0 && (
        <section className="bg-white dark:bg-slate-950 py-16 sm:py-20 lg:py-24 border-t dark:border-slate-800">
          <div className="container mx-auto px-4 max-w-4xl">
            <div className="bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-8 sm:p-12 rounded-3xl">
              <h2 className="text-2xl font-black mb-6 text-slate-900 dark:text-white text-center">Department Achievements</h2>
              <ul className="space-y-4 font-semibold text-sm">
                {achievements.map((item, i) => (
                  <li key={i} className="flex gap-3 items-start">
                    <Users className="w-5 h-5 text-secondary shrink-0 mt-0.5" />
                    <p className="text-gray-700 dark:text-slate-300 leading-relaxed">{item}</p>
                  </li>
                ))}
              </ul>
            </div>
