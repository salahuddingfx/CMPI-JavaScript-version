...
import { AlertCircle, BookOpen, Calculator, Printer, Trophy } from "lucide-react";
...
export function BtebResultCard({ btebResults, query, onCalculateCgpa }) {
        ...
        <div>
          <div className="flex flex-wrap gap-2 items-center">
            ...
            {btebResults.some((r) => (r.exam_type ?? "regular") !== "regular") && (
              <span className="text-[10px] font-black text-amber-700 bg-amber-100 border border-amber-200 px-2 py-0.5 rounded-full animate-pulse">
                      ...
                      <tbody className="divide-y bg-card">
                        {record.referred_subjects?.map((subCode, idx) => (
                          <tr key={subCode} className="hover:bg-red-50/50 dark:hover:bg-red-900/5">
                            ...
                            <td className="px-4 py-2 text-muted-foreground font-medium">{parseSubjectSuffix(subCode)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
