...
import { Menu, X, ChevronDown, ArrowRight, Sun, Moon, User, LogOut } from 'lucide-react';
                    ...
                    <div className="flex flex-col gap-1">
                      {link.dropdown.map((sub) => (
                        <Link
                          key={sub.name}
                          to={sub.path}
                          className="flex items-center justify-between px-4 py-2.5 text-xs font-bold text-slate-600 dark:text-slate-350 rounded-xl hover:bg-primary hover:text-white dark:hover:bg-primary transition-all group/item"
                        >
                          {sub.name}
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-y-0 right-0 z-[110] w-full max-w-[300px] bg-white dark:bg-slate-950 border-l dark:border-slate-900 shadow-2xl p-6 flex flex-col justify-between overflow-y-auto lg:hidden text-left"
            >
              <div>
                {/* Drawer Head */}
                ...
                <div className="flex items-center justify-between gap-3 mb-6 p-2 rounded-2xl bg-slate-50 dark:bg-slate-900 border dark:border-slate-850">
                  <span className="text-xs font-extrabold text-slate-400 pl-2">Theme Switch</span>
                  <button 
                    onClick={toggleTheme}
                                ...
                                >
                                  {link.dropdown.map((sub) => (
                                    <Link
                                      key={sub.name}
                                      to={sub.path}
                                      className="block rounded-xl px-3 py-2 text-xs font-semibold text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-900 hover:text-slate-800 dark:hover:text-white"
                                      onClick={() => setIsOpen(false)}
                                    >
                                      {sub.name}
                                    </Link>
